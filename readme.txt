=== Post Types Converter ===
Contributors: Qassim.Dev
Donate link: http://j.mp/WPTime_donations
Tags: post type, post types, convert, converter, image, images, Post, plugin, posts, page, widget, admin, sidebar, google, twitter, comments, shortcode
Requires at least: 3.0.0
Tested up to: 4.2.2
Stable tag: 1.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

One click convert post type easily, custom post type support.

== Description ==

### Post Types Converter

One click convert post type easily, custom post type support.

### The Features

* Just one click and two options.
* Custom post type support.
* Easy to use.

### Live Demo

* [http://wp-time.com/post-types-converter/](http://wp-time.com/post-types-converter/)

### Rate The Plugin

* [Please rate Post Types Converter plugin](https://wordpress.org/support/view/plugin-reviews/post-types-converter#postform)

### See Also

* [Collection of 87 themes for $69 only.](http://j.mp/ET_WPTime)
* [Request your WordPress plugin starting from $9.](http://j.mp/Request_Plugin_WPTime)
* [Premium WordPress themes on Creative Market.](http://j.mp/CM_WPTime)
* [Premium WordPress themes on Themeforest.](http://j.mp/TF_WPTime)
* [Premium WordPress plugins on Codecanyon.](http://j.mp/CC_WPTime)
* [Unlimited web hosting for $3.95 only.](http://j.mp/BH_WPTime)

### About

* [The plugin designed and developed by Qassim Hassan.](http://qass.im)
* [Qassim Hassan on Twitter.](https://twitter.com/Qassim_Dev)

### More Plugins

* [My Plugins](https://profiles.wordpress.org/qassimdev/#content-plugins)

== Installation ==

### Installation

1. Upload 'post-types-converter' folder to the '/wp-content/plugins/' directory.
2. Activate the plugin through the 'Plugins' menu in WordPress.
3. Go to Plugins menu > Post Types Converter.
4. In "Post Link Or ID" field, enter your post shortlink or post id to convert it to new post type.
5. In "Post Type" field, enter the new post type name to convert your post.
6. Click on "Convert" button.
* [How to get Post Shortlink and Post ID?](http://wp-time.com/post-types-converter/)

== Frequently Asked Questions ==

### FAQ

* [For more questions or help, contact me.](http://wp-time.com/contact/)
* [Or ask me on twitter.](https://twitter.com/Qassim_Dev)

== Changelog ==

= 1.0 =
First version.